...
export const logoutUser = asyncHandler(async (_req, res, _next) => {
  ...
  res.cookie('token', null, {
    secure: process.env.NODE_ENV === 'production' ? true : false,
    // process.env.NODE_ENV == 'production' ? true : false: This checks if the environment is production. If so, it sets the secure flag to true, meaning the cookie will only be sent over HTTPS. If not, it sets the secure flag to false, allowing the cookie to be sent over HTTP.
    maxAge: 0,
    httpOnly: true,
//     httpOnly:

// true: This flag means that the cookie cannot be accessed via JavaScript (e.g., document.cookie), which helps mitigate the risk of cross-site scripting (XSS) attacks. The cookie is only sent in HTTP requests, enhancing security by making it inaccessible to client-side scripts.
  });
